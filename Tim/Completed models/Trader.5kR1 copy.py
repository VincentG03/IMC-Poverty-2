...
class Trader:
        ...
        if len(order_depth.buy_orders) != 0:
            ...
            if prevent_adverse:
                if (best_bid >= fair_value + take_width) and (best_bid_amount <= adverse_volume):
                    ...
                    if quantity > 0:
                        orders.append(Order(product, round(best_bid), -1 * quantity))
    ...
    def market_make(self, product: str, orders: List[Order], bid: int, ask: int, position: int, buy_order_volume: int, sell_order_volume: int) -> Tuple[int, int]:
        ...
        if buy_quantity > 0:
            orders.append(Order(product, int(round(bid)), buy_quantity))  # Buy order

        sell_quantity = self.LIMIT[product] + (position - sell_order_volume)
        if sell_quantity > 0:
            orders.append(Order(product, int(round(ask)), -sell_quantity))  # Sell order
    ...
    def croissants_orders(self, order_depth: OrderDepth, timespan:int, width: float, croissants_take_width: float, position: int, position_limit: int) -> List[Order]:
        ...
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:    
            ...
            # only taking best bid/ask
            buy_order_volume, sell_order_volume = self.take_best_orders(Product.CROISSANTS, fair_value, croissants_take_width, orders, order_depth, position, buy_order_volume, sell_order_volume, True, 250)
            
            # Clear Position Orders
            buy_order_volume, sell_order_volume = self.clear_position_order(Product.CROISSANTS, fair_value, 1.5, orders, order_depth, position, buy_order_volume, sell_order_volume)
            
            aaf = [price for price in order_depth.sell_orders.keys() if price > fair_value + 0.75]
            bbf = [price for price in order_depth.buy_orders.keys() if price < fair_value - 0.75]
            baaf = min(aaf) if len(aaf) > 0 else fair_value + 1.5
            bbbf = max(bbf) if len(bbf) > 0 else fair_value - 1.5

            # Market Make
            buy_order_volume, sell_order_volume = self.market_make(Product.CROISSANTS, orders, bbbf + 1.5, baaf - 1.5, position, buy_order_volume, sell_order_volume)
    ...
    def jams_orders(self, order_depth: OrderDepth, timespan:int, width: float, jams_take_width: float, position: int, position_limit: int) -> List[Order]:
        ...
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:    
            ...
            # only taking best bid/ask
            buy_order_volume, sell_order_volume = self.take_best_orders(Product.JAMS, fair_value, jams_take_width, orders, order_depth, position, buy_order_volume, sell_order_volume, True, 470)
            
            # Clear Position Orders
            buy_order_volume, sell_order_volume = self.clear_position_order(Product.JAMS, fair_value, 1.5, orders, order_depth, position, buy_order_volume, sell_order_volume)
            ...
            # Market Make
            buy_order_volume, sell_order_volume = self.market_make(Product.JAMS, orders, bbbf + 1.5, baaf - 1.5, position, buy_order_volume, sell_order_volume)
    ...
    def djembes_orders(self, order_depth: OrderDepth, timespan:int, width: float, djembes_take_width: float, position: int, position_limit: int) -> List[Order]:
        ...
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:    
            ...
            # only taking best bid/ask
            buy_order_volume, sell_order_volume = self.take_best_orders(Product.DJEMBES, fair_value, djembes_take_width, orders, order_depth, position, buy_order_volume, sell_order_volume, True, 130)
            
            # Clear Position Orders
            buy_order_volume, sell_order_volume = self.clear_position_order(Product.DJEMBES, fair_value, 1.5, orders, order_depth, position, buy_order_volume, sell_order_volume)
            ...
            baaf = min(aaf) if len(aaf) > 0 else fair_value + 0.75
            bbbf = max(bbf) if len(bbf) > 0 else fair_value - 0.75

            # Market Make
            buy_order_volume, sell_order_volume = self.market_make(Product.DJEMBES, orders, bbbf + 1.5, baaf - 1.5, position, buy_order_volume, sell_order_volume)
    ...
    def run(self, state: TradingState):
        ...
        kelp_make_width = 2#3.5
        kelp_take_width = 1
        kelp_position_limit = 50
        kelp_timemspan = 10
        
        squid_ink_make_width = 2#3.5
        squid_ink_take_width = 1
        squid_ink_position_limit = 50
        squid_ink_timemspan = 10

        croissants_make_width = 1#3.5
        croissants_take_width = 1
        croissants_position_limit = 250
        croissants_timemspan = 10

        jams_make_width = 1#3.5
        jams_take_width = 1
        jams_position_limit = 350
        jams_timemspan = 10

        djembes_make_width = 1#3.5
        djembes_take_width = 1
        ...
        traderData = jsonpickle.encode( { "kelp_prices": self.kelp_prices, "kelp_vwap": self.kelp_vwap, "squid_ink_prices": self.squid_ink_prices, "croissant_vwap": self.croissants_vwap, "croissants_prices": self.croissants_prices, "jams_vwap": self.jams_vwap, "jams_prices": self.jams_prices, "djembes_vwap": self.djembes_vwap, "djembes_prices": self.djembes_prices })
