...
class Trader:
    ...
    def compute_momentum(self, price_list: List[float]) -> float:
        if len(price_list) < 2:
            return 0.0
        return price_list[-1] - price_list[-2]

    def compute_time_sine(self, timestamp: int) -> float:
        # Assuming the trading day is 100000 ms long
        theta = 2 * np.pi * (timestamp % 100000) / 100000
        return np.sin(theta)

    def compute_order_size(self, momentum: float, sine_wave: float, max_size: int = 20) -> int:
        # Very basic LR-inspired model: linear combo of features
        score = 0.8 * momentum + 0.4 * sine_wave
        return int(np.clip(score * max_size, -max_size, max_size))
    ...
    def kelp_orders(self, order_depth: OrderDepth, timespan:int, width: float, kelp_take_width: float, position: int, position_limit: int,timestamp: int) -> List[Order]:
        orders: List[Order] = []

        buy_order_volume = 0
        sell_order_volume = 0
        sine_wave = self.compute_time_sine(timestamp)

        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:    

            # Compute features
            momentum = self.compute_momentum(self.kelp_prices)
            sine_wave = self.compute_time_sine(timestamp)
            order_size = self.compute_order_size(momentum, sine_wave)

            # Decide trade
            fair_value = mmmid_price
            if order_size > 0:
                best_ask = min(order_depth.sell_orders.keys())
                volume = min(order_size, self.LIMIT[Product.KELP] - position)
                if best_ask <= fair_value + 2:
                    orders.append(Order(Product.KELP, best_ask, volume))
            elif order_size < 0:
                best_bid = max(order_depth.buy_orders.keys())
                volume = min(abs(order_size), self.LIMIT[Product.KELP] + position)
                if best_bid >= fair_value - 2:
                    orders.append(Order(Product.KELP, best_bid, -volume))
    ...
    def squid_ink_orders(self, order_depth: OrderDepth, timespan:int, width: float, kelp_take_width: float, position: int, position_limit: int,timestamp: int) -> List[Order]:
        orders: List[Order] = []
        ...
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:    
            # Compute features
            momentum = self.compute_momentum(self.squid_ink_prices)
            sine_wave = self.compute_time_sine(timestamp)
            order_size = self.compute_order_size(momentum, sine_wave)
            ...
            if order_size > 0:
                best_ask = min(order_depth.sell_orders.keys())
                volume = min(order_size, self.LIMIT[Product.SQUID_INK] - position)
                if best_ask <= fair_value + 2:
                    orders.append(Order(Product.SQUID_INK, best_ask, volume))
            elif order_size < 0:
                best_bid = max(order_depth.buy_orders.keys())
                volume = min(abs(order_size), self.LIMIT[Product.SQUID_INK] + position)
                if best_bid >= fair_value - 2:
                    orders.append(Order(Product.KELP, best_bid, -volume))
    ...
    def run(self, state: TradingState):
        ...
        squid_ink_make_width = 2.00
        squid_ink_take_width = 1.5
        squid_ink_position_limit = 50
        squid_ink_timemspan = 8
        ...
        if Product.KELP in state.order_depths:
            ...
            kelp_orders = self.kelp_orders(state.order_depths[Product.KELP], kelp_timemspan, kelp_make_width, kelp_take_width, kelp_position, kelp_position_limit,state.timestamp)
        ...
        if Product.SQUID_INK in state.order_depths:
            ...
            squid_ink_orders = self.squid_ink_orders(state.order_depths[Product.SQUID_INK], squid_ink_timemspan, squid_ink_make_width, squid_ink_take_width, squid_ink_position, squid_ink_position_limit,state.timestamp)
            result[Product.SQUID_INK] = squid_ink_orders

        
        traderData = jsonpickle.encode( { "kelp_prices": self.kelp_prices, "kelp_vwap": self.kelp_vwap, "squid_ink_prices": self.squid_ink_prices, "squid_ink_vwap": self.squid_ink_vwap })
