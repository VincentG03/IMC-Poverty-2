...
empty_dict = {'RAINFOREST_RESIN' : 0, 'KELP' : 0} #, 'COCONUTS' : 0, 'PINA_COLADAS' : 0, 'BERRIES' : 0, 'DIVING_GEAR' : 0, 'DIP' : 0, 'BAGUETTE': 0, 'UKULELE' : 0, 'PICNIC_BASKET' : 0}
...
class Trader:
    ...
    POSITION_LIMIT = {'RAINFOREST_RESIN' : 20, 'KELP' : 20} # 'COCONUTS' : 600, 'PINA_COLADAS' : 300, 'BERRIES' : 250, 'DIVING_GEAR' : 50, 'DIP' : 300, 'BAGUETTE': 150, 'UKULELE' : 70, 'PICNIC_BASKET' : 70}
    ...
    def calc_next_price_KELP(self):
        # KELP cache stores price from 1 day ago, current day resp
    ...
    def compute_orders_pearls(self, product, order_depth, acc_bid, acc_ask):
        ...
        if (cpos < self.POSITION_LIMIT['RAINFOREST_RESIN']) and (self.position[product] < 0):
            num = min(40, self.POSITION_LIMIT['RAINFOREST_RESIN'] - cpos)
            orders.append(Order(product, min(undercut_buy + 1, acc_bid-1), num))
            cpos += num

        if (cpos < self.POSITION_LIMIT['RAINFOREST_RESIN']) and (self.position[product] > 15):
            num = min(40, self.POSITION_LIMIT['RAINFOREST_RESIN'] - cpos)
            orders.append(Order(product, min(undercut_buy - 1, acc_bid-1), num))
            cpos += num
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        result = {'RAINFOREST_RESIN' : [], 'KELP' : []} #, 'COCONUTS' : [], 'PINA_COLADAS' : [], 'DIVING_GEAR' : [], 'BERRIES' : [], 'DIP' : [], 'BAGUETTE' : [], 'UKULELE' : [], 'PICNIC_BASKET' : []}
        ...
        _, bs_KELP = self.values_extract(collections.OrderedDict(sorted(state.order_depths['KELP'].sell_orders.items())))
        _, bb_KELP = self.values_extract(collections.OrderedDict(sorted(state.order_depths['KELP'].buy_orders.items(), reverse=True)), 1)
        ...
        RAINFOREST_RESIN_lb = 10000
        RAINFOREST_RESIN_ub = 10000
